...
class Wave3: public System
{
public:
    Wave3(vector <vector <vector <float>>> initial_state, vector <float> params, float dt, float dx, long step): System(initial_state, params, dt, dx, step) {};
    virtual ~Wave3() {};
    vector <vector <float>> f()
    {
        vector <float> initial_state_x=initial_state[0][step];
        vector <float> u1= prev(initial_state_x, 2);
        vector <float> u2= prev(initial_state_x, 1);
        vector <float> u3= next(initial_state_x, 1);
        vector <float> u4= initial_state_x;
        vector <float> a1=(-1)*u1;
        vector <float> a2=u3;
        vector <float> a3=3*u2;
        vector <float> a4=(-3)*u4;
        vector <float> a=a1;
        a=a+a2;
        a=a+a3;
        a=a+a4;
        a=(-1)*(pow(params[0], 3)*pow(dt, 2))/(6*pow(dx, 3))*a;
        vector <float> b1=u3;
        vector <float> b2=u2;
        vector <float> b3=(-2)*u4;
        vector <float> b=b1;
        b=b+b2;
        b=b+b3;
        b=(pow(params[0], 2)*dt)/(2*pow(dx, 2))*b;
        vector <float> c1=u1;
        vector <float> c2=2*u3;
        vector <float> c3=(-6)*u2;
        vector <float> c4=3*u4;
        vector <float> c=c1;
        c=c+c2;
        c=c+c3;
        c=c+c4;
        c=(-1)*params[0]/(6*dx)*c;
        vector <float> der=a+b;
        der=der+c;
        station_derivative.push_back(der);
...
class WaveRefl: public System
{
public:
    WaveRefl(vector <vector <vector <float>>> initial_state, vector <float> params, float dt, float dx, long step): System(initial_state, params, dt, dx, step) {};
    virtual ~WaveRefl() {};
    vector <vector <float>> f()
    {
        vector <float> initial_state_x1=initial_state[0][step];
...
public:
    ...
    void integrate()
    {
        for (long i=1; i<iterations; i++)
        {
            ...
            {
                for (long k=0; k<states[j][i].size(); k++)
                {
...
int main(int argc, char* argv[])
{
    ...
    f1.close();
    ofstream f2(argv[2]);
    Wave3 w(initial_state, params, dt, dx, 0);
    Euler_method <Wave3> Ew(w, T, X);
